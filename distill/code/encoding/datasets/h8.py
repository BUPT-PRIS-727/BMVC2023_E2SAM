...
def default_loader(id, root,split):
    ...
    npy = np.load(os.path.join(root,'{}.npz').format(id))['x'].astype('uint8')
    ...
    img = np.array(npy, np.float32)/255.0-0.5
...
class H8Set(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if("train" in self.split):
            ...
            output=transform.RandomCrop(512)(img)
            # img=output[[2,3,13],:,:]
            img=output[:16,:,:]
            # mask=output[16,:,:]
        else:
            ...
            # img=torch.concat(tuple(split_img),dim=0)[:,[2,3,13]]

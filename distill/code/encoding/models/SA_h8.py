...
def get_h8_512(checkpoint=None,criterion_seg=None):
...
class SA(nn.Module):
    ...
    def forward(self,x: torch.Tensor,y: torch.Tensor,channels,epoch=None) -> torch.Tensor:
        if len(x.shape)==5:
            B,_,C,H,W=x.shape
        ...
        if self.training:
            teacher_x=x
            with torch.no_grad():
                teacher_feat=self.teacher_encoder(teacher_x[:,channels,:,:])
                teacher_feat=self.teacher_conv(teacher_feat.permute(0, 3, 1, 2))

            student_feat_list=[]            
            for i in channels:
                student_feat_list.append(self.student_split[i](student_feat))
            student_feat_combine=torch.cat(tuple(student_feat_list),dim=-1)
            # student_feat_combine=self.student_combine(student_feat_combine).permute(0, 3, 1, 2)
            student_feat_combine=self.student_conv(student_feat_combine.permute(0, 3, 1, 2))
            with torch.no_grad():
                teacher_out=self.share_encoder(teacher_feat)
            student_out=self.share_encoder(student_feat_combine)
        if self.criterion is not None and self.training:

            loss1= self.criterion(student_out,teacher_out)
            return {'f_loss':loss1,'rec_loss':loss1}

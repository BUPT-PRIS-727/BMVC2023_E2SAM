...
from encoding.nn import BootstrappedCELoss, OhemCELoss, SegmentationLosses, dice_ce_loss,focal_loss,ce_loss,bce_loss,MMD
from encoding.utils import (AverageMeter, LR_Scheduler,
                            intersectionAndUnionGPU, save_checkpoint)
from option_distill import Options
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        try:
            ...
            # validate(val_loader, model, nn.CrossEntropyLoss(), 2)
...
def train(train_loader, model, scaler, optimizer, epoch, scheduler, nclass,currentSteps,wandbHandle=None):
    ...
    channel_meter=[AverageMeter() for i in range(16)]
    ...
    for i, (input,target) in enumerate(train_loader):
        B=input.shape[0]
        input=input+torch.randn_like(input)*0.2
        # import pdb;pdb.set_trace()
        currentSteps[0]+=1
        ...
        # with autocast():
        with autocast():
            main_loss = model(input,target, selected_channels,epoch=vis) ##jpu
            vis=None
            ...
            # loss = main_loss['f_loss']+main_loss['rec_loss'] #+ 0.25 * 0.4 * jpu_loss
            loss = main_loss['f_loss'] #+ 0.25 * 0.4 * jpu_loss
...
def validate(val_loader, model, criterion, nclass):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        B,_,C,H,W=input.shape
        target=target.contiguous().reshape(-1,512,512)
        with torch.no_grad():
            output = model(input,target,[0,1,2],epoch=None)
        # if args.zoom_factor != 8:
        #     output = F.interpolate(output, size=target.size()[1:], mode='bilinear', align_corners=True)
        loss = criterion(output.view(B,2,-1),target.view(B,-1).long())

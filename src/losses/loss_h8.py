...
class DiceLoss(nn.Module):
    ...
    def forward(self, predict, target):
        # target = make_one_hot(target, 11)[:, :10, :, :]
        assert predict.shape == target.shape, 'predict & target shape do not match'
        total_loss = 0
        predict = F.softmax(predict, dim=1) # b, 10, h, w
...
class dice_ce_loss(nn.Module):
    ...
    def one_hot(self, y_pred, y_true):
        ...
        y_one_hot = F.one_hot(y_true, 11)[:, :, :, :10].permute(0, 3, 1, 2) # one hot class=11, ignore 11th channel
        ...
        return self.sigmoid(y_pred), y_one_hot
    ...
    def smooth_l1(self, y_pred, y_true):
        y_pred, y_one_hot = self.one_hot(y_pred, y_true)
        loss = self.l1_loss(y_pred, y_one_hot)
        return loss
    ...
    def forward(self, y_pred, y_true):
        ...
        # y_pred, y_one_hot = self.one_hot(y_pred, y_true)
        # b = self.dice_loss(y_pred, y_one_hot)
        # c = self.smooth_l1(y_pred, y_true)
        return a + b

...
class SA(BaseModel):
    ...
    def forward(self,x: torch.Tensor,y: torch.Tensor,mode=None) -> torch.Tensor:
        ...
        # teacher_x=[x[i:i+1,channel,:,:] for i,channel in enumerate(channels)]
        # teacher_x=torch.cat(tuple(student_feat_list),dim=-1)
        # with torch.no_grad():
        #     teacher_out=self.share_encoder(teacher_feat)
        # student_out=self.share_encoder(student_feat)
